"""
train_ls14pp.py — Modelo LS14++ (Plano SILVER)
----------------------------------------------
Baseado em janela de 25 concursos.
Produz: models/ls14pp/ls14pp_final.keras
"""
...
MODELS = os.path.join(ROOT, "models", "ls14pp")
...
def main():
    ...
    out_path = os.path.join(MODELS, "ls14pp_final.keras")
    model.save(out_path)
    ...
    with open(os.path.join(SUMMARIES, "lf_train_summary.txt"), "a", encoding="utf-8") as f:
        f.write(
            f"DATE={now} | MODEL=ls14pp | PLAN=Silver | PATH={out_path} | "
